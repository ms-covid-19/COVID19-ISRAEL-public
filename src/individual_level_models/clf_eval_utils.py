...
def quickplot_eval_2(y_true, y_pred, model_name, p_high=1.0, out_dir=MODELS_DIR):
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, '{}_quickplot_eval_2'.format(model_name)))
    plt.close()

def quickplot_eval_3(y_true, y_pred, model_name, p_high=1.0, out_dir=MODELS_DIR):
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, '{}_quickplot_eval_3'.format(model_name)))
    plt.close()


def quickplot_eval_4(y_true, y_pred, model_name, p_high=1.0, out_dir=MODELS_DIR):
    ...
    fig.savefig(os.path.join(out_dir, '{}_quickplot_eval_4'.format(model_name)))


# Feature importance - Non tree models #
def feature_importance_plot(model, X_train, model_name, out_dir=MODELS_DIR):
    importances = model.coef_.ravel()
    indices = np.argsort(importances).ravel()
    fig, ax = plt.subplots(1, 1, figsize=(10, len(importances) // 2))
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(out_dir, '{}_feature_importance.png'.format(model_name)))
